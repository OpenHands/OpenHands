"""web_zap_spider — passive ZAP spider."""
...
async def _default_runner(target: str) -> list[dict[str, Any]]:
    return [
        stub_finding(
            title="ZAP spider discovered URL",
            severity="info",
            asset=target,
            endpoint="/",
            extra={"tool": "zap_spider"},
        )
    ]


async def run_zap_spider(
    *,
    target: str,
    engagement_id: str,
    ...
    for item in items:
        payload = normalize_finding(
            ...
            description="ZAP spider (passive)",
        )
        posted.append(await client.post_finding(payload))
    return ok({"tool": "web_zap_spider", "findings": posted})
