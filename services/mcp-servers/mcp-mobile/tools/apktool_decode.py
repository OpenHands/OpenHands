...
async def _default_runner(apk: Path, out_dir: Path) -> tuple[int, str, str]:
    return await run_cmd(
        "apktool",
        ["d", "-f", "-o", str(out_dir), str(apk)],
        use_real=use_real_binaries(),
        timeout=300.0,
    )


async def run_apktool_decode(
    *,
    engagement_id: str,
    apk_path: str,
    ...
    try:
        ...
        out = resolve_workspace_path(out_dir or f"{resolved.stem}_apktool")
    ...
    if use_real_binaries() and code != 0:
        return err("apktool_failed", message=(stderr or stdout)[:300])
    return ok(
        {
            "tool": "mobile_apktool_decode",
            "engagement_id": engagement_id,
            "apk_path": str(resolved),
            "out_dir": str(out),
            "output": (stdout or "decoded").strip(),
        }
    )
