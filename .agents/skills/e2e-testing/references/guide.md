...
- **Atomic journeys**: The conversation, ACP, model-switch, and automation lifecycle specs each use one `test()` with named `test.step()` phases. Files/git panel checks and project/user/skill-deletion checks remain independently runnable tests. These six specs use `utils/atomic-journey.ts` for test-scoped profile names, conversation tracking, restoration of the default agent and active profiles, resource cleanup, and mock trajectory reset. They do not use `serial` dependencies or predecessor-ID skips; `workers: 1` still protects the shared backend/trajectory. Register cleanup before creating resources so partial failures are covered. Cleanup errors fail the owning test while all remaining cleanup actions are attempted. Skill deletion creates and verifies its own fixture before removing it. The npm user-skill fixture path follows the launcher's `OH_PERSISTENCE_DIR` (`dirname(STATE_DIR)/skills`); Docker retains its mounted fixture override. Runner-wide isolation is tracked separately in #15485.
...
- **Mock LLM server** (`tests/e2e/mock-llm/scripts/mock-llm-server.py`): Python HTTP server using openhands-sdk's `TestLLM` to return scripted tool-call + text trajectories. Supports admin API endpoints for dynamic trajectory management:
  ...
  - Automatic titles: requests matching the SDK title-generation system/user prompt (without tools) receive a static title, without consuming a scripted turn or appearing in `/admin/requests`. Title generation runs concurrently with the agent loop, so padding a trajectory cannot reliably reserve a response for it. A standard-library HTTP regression test lives in `scripts/test_mock_llm_server.py` and runs in both mock-LLM workflows.
...
- **Trajectory ownership**: The six atomic journeys script only agent turns; profile-validation pings and automatic-title calls receive separate canned replies. Do not add blank padding to guess when a concurrent title request will arrive. If a new internal LLM operation consumes a trajectory, identify it from the captured request before changing the budget. Since `openhands-automation==1.10.0` (openhands/automation#405), the automation lifecycle scripts the required `finish` tool turn and final text reply for its run conversation. The mock server's "Mock LLM exhausted after N calls" diagnostics identify budget drift.
