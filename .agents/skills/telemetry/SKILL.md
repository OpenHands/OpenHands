...
name: telemetry
description: >
  PostHog telemetry and analytics architecture for agent-canvas — the single
  Canvas-owned client, consent control, funnel events, and the event dictionary.
  Load before touching telemetry.ts, TelemetryProvider, consent, tracking
  events, or cloud funnel observability.
triggers:
  - telemetry
  - analytics
  - posthog
