...
- `tools/mcp_legacy_schema_compat.py` fixes OpenHands/OpenHands#17615 ("Old conversations cannot be loaded after upgrading to 1.21.0"). The 1.21.0 `fastmcp<4` pin downgrades the transitive `mcp` package from 2.x to 1.x; the two majors spell several MCP wire fields differently (`Tool.input_schema` ↔ `inputSchema`, `CallToolResult.is_error` ↔ `isError`, `mime_type` ↔ `mimeType`, …) and 1.x rejects what 2.x wrote, so a `SystemPromptEvent` carrying an `MCPToolDefinition` fails to restore. The module adds an `AliasChoices` of both spellings to every `mcp.types` field, rebuilds those models, then force-rebuilds the already-imported `openhands`/`mcp` pydantic models that embed them (pydantic bakes validators at class construction, so patching `mcp.types.Tool` alone leaves `MCPToolDefinition` holding the old validator). Opening the SDK event graph itself here would register tools as an import side effect, so unimported models pick up the patched fields when built later. It runs at import time and is a no-op under `mcp` 2.x; remove it once the pinned agent-server reads both spellings natively. `AGENT_SERVER_IMPORT_MODULES` in `scripts/dev-safe.mjs` and `AGENT_SERVER_IMPORT_MODULES` in `docker/entrypoint.sh` both list it **before** `canvas_ui_tool` so the SDK builds on patched fields rather than the recompile path. Coverage: `tools/tests/test_mcp_legacy_schema_compat.py` (behavioral, pytest — run by `.github/workflows/ci-script-tests.yml` against the pinned `openhands-sdk`) and `__tests__/scripts/mcp-legacy-schema-compat-wiring.test.ts` (wiring drift between the two launch paths).
