...
- Key rotation: `LOCAL_BACKEND_API_KEY` is an auth credential and users do rotate it, so nothing encrypted at rest may be derived from it. `AUTOMATION_KV_SECRET` — which the automation backend uses both to sign per-run KV tokens and to encrypt automation KV state — is resolved by `resolveAutomationKvSecret()` in `scripts/dev-with-automation.mjs` (reused by `dev-static.mjs`) and by the `# >>> automation-kv-secret` block in `docker/entrypoint.sh`: explicit env, else the persisted `<stateDir>/automation-kv-secret.txt`, else the current session key seeded into that file so stacks carrying KV state written under the old session-key default keep reading it. Coverage lives in `__tests__/scripts/dev-with-automation.test.ts`, `__tests__/scripts/dev-static.test.ts`, and `__tests__/scripts/docker-automation-kv-secret.test.ts`; the user-facing rotation checklist is section 6 of `docs/SELF_HOSTING.md`.
