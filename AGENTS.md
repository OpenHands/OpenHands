...
- **Dual-stack binding**: `scripts/static-server.mjs` and `scripts/ingress.mjs` default to `127.0.0.1` (loopback) so a local session key is not exposed on the LAN. The Docker entrypoint still passes `--host ::` (dual-stack, all interfaces inside the container) plus `--allow-lan-session-key` so published ports keep working. Playwright npm configs should use `http://127.0.0.1` for the canvas origin when talking to the default bind. The mock LLM server URL (`MOCK_LLM_URL`) still uses `127.0.0.1` because the Python mock server is a separate process whose bind behavior we don't control.
