...
- Docker packaging uses named targets in `docker/Dockerfile`. `enterprise-sandbox` and `dev-sandbox` inherit the pinned upstream Agent Server `ENTRYPOINT`/`CMD` unchanged; neither target may copy Canvas, Automation, or `docker/entrypoint.sh`. `control-plane` runs Canvas, Automation, and the proxy against an explicit external `AGENT_SERVER_URL`. The final `final` target remains the legacy all-in-one compatibility image. `docker/compose.yml` and `scripts/docker-dev.mjs` own the separated local topology; `scripts/docker-build.mjs` owns deterministic target selection. The Enterprise sandbox is `linux/amd64` only and must be rebuilt from the Agent Server tag expected by each OpenHands Enterprise release.
