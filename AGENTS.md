...
- Provider Connections (issue #15492): `src/api/provider-connections-service.ts` follows the raw-`fetch` precedent of `llm-balance-service.ts`/`llm-subscription-service.ts` (local-only; throws `ProviderConnectionsNotOnCloudError` on cloud). Path constants live in `src/constants/provider-connections.ts` — keep the literal `"/api/..."` strings there, never in the service file, because `src/api/no-direct-agent-server-calls.test.ts` greps for `fetch(` followed within 200 chars by an `/api/` literal (a stray `fetch(` in a comment trips it). The `ProviderConnectionsSection` gates itself off on cloud via `assertConnectionsSupportedLocally()` and renders nothing there. i18n keys are `SETTINGS$CONNECTION_*` / `SETTINGS$PROVIDER_CONNECTIONS*` in `translation.json` (15 languages; `declaration.ts` + `public/locales/` are gitignore'd generated artifacts — run `npm run make-i18n` to regenerate). Tests that render `LlmSettingsLocalView` must mock `useProviderConnections` + `useCanManageOrgProfiles` + the wizard to avoid a competing `loading-spinner` testid.
