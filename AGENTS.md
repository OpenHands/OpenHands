...
- Home page workspace UX (local backend):
  ...
  - The conversation-panel new-thread picker (`LocalNewConversationMenu` / `CloudNewConversationMenu` in `src/components/features/conversation-panel/`, mounted from `conversation-panel-new-thread-picker.tsx`) opens a popover that is a **flat list**, not the home-screen combobox: a leading "No workspace" entry plus one entry per stored workspace, each clicking through to `useCreateConversation` immediately (no separate Launch button). It mirrors the dropdown footer actions/pattern (`+ Add Workspace`, `Manage Workspaces`) locally rather than embedding `WorkspaceDropdown` itself. The sidebar "+ New Chat" control is a nav link to `/conversations`, not this popover.
