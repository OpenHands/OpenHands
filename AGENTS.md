...
- Unbounded SDK transitive deps are pinned in `config/defaults.json` under `constraints` and appended by `pushTransitiveDepConstraints()` in `scripts/dev-safe.mjs`. Only the released-PyPI branches get them: `OH_AGENT_SERVER_LOCAL_PATH` / `OH_AGENT_SERVER_GIT_REF` deliberately skip the pins, because those modes are how an SDK fix that removes a pin gets tested. Current pins are `agent-client-protocol<0.11` and `fastmcp>=3.2.0,<4`. The fastmcp bound keeps MCP OAuth working: below 3.2.0 FastMCP never loads the persisted token expiry, so `is_token_valid()` returns True for an expired access token and the refresh grant is skipped; from 4.0.0 `callback_handler()` returns an `AuthorizationCodeResult` instead of a tuple, so the agent-server's forked callback raises `'tuple' object has no attribute 'state'` and re-authorization fails (issue #17077, SDK #4818/#4821). Docker does not need the pin — the agent-server image builds from the SDK's `uv.lock`. Drop a pin only once the SDK ships its own ceiling.
