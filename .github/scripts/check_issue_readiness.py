...
# Markdown fenced code block opener. Three or more backticks/tildes at the start
# of a line (up to 3 spaces indentation), optionally followed by a language
# specifier. We capture only the delimiter to match against on close.
FENCE_OPEN_RE = re.compile(r"(?m)^( {0,3})(`{3,}|~{3,})")
...
def strip_fenced_blocks(body: str) -> str:
    """Remove fenced code blocks so headings inside them cannot be parsed."""
    ...
    for line in lines:
        ...
        if m is not None and m.group(2):
            ...
            length = len(delim)
            if fence_delim == "":
                # Opening fence — skip this line and everything until close.
                fence_delim = delim
                fence_len = length
                continue
            elif delim == fence_delim and length >= fence_len:
                # Closing fence — leave fenced state, skip this line too.
                fence_delim = ""
                fence_len = 0
                continue
            # else: mismatched fence, keep going.
        elif fence_delim:
            # Inside an active fence — skip this line.
            continue
...
def extract_sections(body: str) -> dict[str, str]:
    ...
    Fenced code blocks are stripped before heading extraction so that quoted
    template headings do not manufacture spurious sections, and fenced log
    snippets do not truncate a real section.
    """
    clean_body = strip_fenced_blocks(body)
    matches = list(HEADING_RE.finditer(clean_body))
    sections: dict[str, str] = {}
    for index, match in enumerate(matches):
        start = match.end()
        end = matches[index + 1].start() if index + 1 < len(matches) else len(clean_body)
        sections[match.group(1).strip().lower()] = clean_body[start:end]
    return sections
