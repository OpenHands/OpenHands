...
def has_reproduction_command(text: str) -> bool:
    """True when *text* shows a command and its observable output.

    Accepts non-UI evidence: a reproduction command (npx/npm/pnpm/uv/python/bash
    invoking install/run/execute/test) alongside output that signals failure
    (error/fail/traceback/non-zero-exit).
    """
    has_command = bool(
        re.search(
            r"(?:^|\n)\s*\$?\s*(?:npx|npm|pnpm|uv|python|python3|bash|sh)\b",
            text,
        )
    )
    has_output = bool(
        re.search(
            ...
            r"non-zero|exit code [1-9]|\b\d+ errors?\b)",
            text,
        )
    )
...
def check_bug(sections: dict[str, str]) -> ReadinessResult:
    ...
    if not actual:
        result.add(
            "Fill in the `### Actual Behavior` section demonstrating the bug."
        )
    else:
        # UI evidence: run method + screenshot/video
        has_ui_evidence = references_run_method(actual) and has_screenshot_or_video(actual)
        # Non-UI evidence: reproduction command + output
        has_repro_evidence = has_reproduction_command(actual)

        if not has_ui_evidence and not has_repro_evidence:
            if not references_run_method(actual):
                result.add(
                    "For a UI bug: reference a run method (`agent-canvas`, "
                    "`npm run`, or `app.all-hands.dev/canvas`) in the Actual "
                    "Behavior section. For a non-UI bug: include a reproduction "
                    "command and its output."
                )
            if not has_screenshot_or_video(actual):
                result.add(
                    "For a UI bug: include a screenshot or video of the bug "
                    "in the Actual Behavior section. For a non-UI bug: include "
                    "the command output showing the failure."
                )
