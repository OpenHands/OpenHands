...
# Bedrock utility functions (inlined from removed openhands.llm.bedrock module)
def _bedrock_list_foundation_models(
...
def _bedrock_remove_error_modelId(model_list: list[str]) -> list[str]:
    """Remove bedrock models from list to avoid errors."""
...
def get_supported_llm_models(
    ...
    * the recommended default model.
    ...
    litellm_model_list_without_bedrock = _bedrock_remove_error_modelId(
        litellm_model_list
    )
    ...
    if (
        ...
        bedrock_model_list = _bedrock_list_foundation_models(
            llm_config.aws_region_name,
