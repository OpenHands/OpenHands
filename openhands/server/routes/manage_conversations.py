...
async def _try_delete_v1_conversation(
    ...
    try:
        ...
        if app_conversation_info:
            ...
            # Only delete the sandbox if no other conversation still uses it
            if sandbox_id and not sandbox_is_shared:
                # Re-check after deletion in case of concurrent deletes
                remaining = await app_conversation_info_service.count_conversations_by_sandbox_id(
                    sandbox_id
                )
                if remaining == 0:
                    asyncio.create_task(
