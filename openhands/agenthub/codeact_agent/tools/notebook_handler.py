...
class NotebookHandler:
    ...
    def parse_notebook(self, file_path: str) -> str:
        """Parse notebook file and return formatted content."""
        path = Path(file_path)
        try:
            content = self.process_notebook(path)
            return content
        except json.JSONDecodeError as e:
            raise ValueError(f"Invalid Jupyter notebook JSON: {e}")
        except Exception as e:
            raise ValueError(f"Error processing notebook: {e}")
