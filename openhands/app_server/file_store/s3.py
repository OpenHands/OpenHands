...
class S3FileStore(FileStore):
    ...
    def bucket_name(self) -> str:
        ...
        if self._resolved_bucket is None:
            self._resolved_bucket = self.bucket or os.environ['AWS_S3_BUCKET']
    ...
    def write(self, path: str, contents: str | bytes) -> None:
        try:
            ...
            self.client.put_object(Bucket=self.bucket_name, Key=path, Body=as_bytes)
        except botocore.exceptions.ClientError as e:
            if e.response['Error']['Code'] == 'AccessDenied':
                raise FileNotFoundError(
                    f"Error: Access denied to bucket '{self.bucket_name}'."
                )
            elif e.response['Error']['Code'] == 'NoSuchBucket':
                raise FileNotFoundError(
                    f"Error: The bucket '{self.bucket_name}' does not exist."
                )
            raise FileNotFoundError(
                f"Error: Failed to write to bucket '{self.bucket_name}' at path {path}: {e}"
            )
    ...
    def read(self, path: str) -> str:
        try:
            ...
            if e.response['Error']['Code'] == 'NoSuchBucket':
                ...
                raise FileNotFoundError(
                    f"Error: The object key '{path}' does not exist in bucket '{self.bucket_name}'."
                )
            else:
                raise FileNotFoundError(
                    f"Error: Failed to read from bucket '{self.bucket_name}' at path {path}: {e}"
                )
    ...
    def delete(self, path: str) -> None:
        try:
            ...
            response = self.client.list_objects_v2(
                Bucket=self.bucket_name, Prefix=f'{path}/'
            )
            ...
            if e.response['Error']['Code'] == 'NoSuchBucket':
                ...
                raise FileNotFoundError(
                    f"Error: Failed to delete key '{path}' from bucket '{self.bucket_name}': {e}"
                )
        except Exception as e:
            raise FileNotFoundError(
                f"Error: Failed to delete key '{path}' from bucket '{self.bucket_name}: {e}"
