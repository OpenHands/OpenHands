...
class RemoteSandboxService(SandboxService):
    ...
    async def start_sandbox(
        ...
        try:
            # Enforce sandbox limits by cleaning up old sandboxes; non-fatal
            try:
                await self.pause_old_sandboxes(self.max_num_sandboxes - 1)
            except Exception as _limit_exc:
                _logger.warning(
                    'Failed to enforce sandbox limits before start: %s', _limit_exc
                )
            # Get sandbox spec
    ...
    async def resume_sandbox(self, sandbox_id: str) -> bool:
        ...
        try:
            ...
            _logger.warning(
                'Failed to enforce sandbox limits before resume: %s', _limit_exc
            )
    ...
    async def pause_old_sandboxes(self, max_num_sandboxes: int) -> list[str]:
        ...
        running_session_ids = [
            ...
            for runtime in content.get('runtimes', [])
...
async def poll_agent_servers(api_url: str, api_key: str, sleep_interval: int):
    """When the app server does not have a public facing url, poll the agent servers.
