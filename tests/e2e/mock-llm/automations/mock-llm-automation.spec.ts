...
/** Poll the automation backend until an automation named AUTOMATION_NAME appears. */
async function waitForCreatedAutomationId(
  ...
  while (Date.now() < deadline) {
    ...
    if (resp?.ok()) {
      ...
      const match = automations.find((a) => a.name === AUTOMATION_NAME);
