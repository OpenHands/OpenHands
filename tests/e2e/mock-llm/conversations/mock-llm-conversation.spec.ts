...
test("create and activate an LLM profile, run and resume a conversation", async ({
  page,
  request,
  journey,
}) => {
  test.setTimeout(180_000);
  const PROFILE_NAME = journey.profileName;
  let createdConversationId = "";
  await test.step("create an LLM profile pointing at the mock LLM server", async () => {
  ...
  await test.step("activate the mock-llm profile and verify settings API", async () => {
    await routeSessionApiKey(page);
    await page.goto("/settings/llm", { waitUntil: "domcontentloaded" });
    await dismissAnalyticsModal(page);
    await waitForTestId(page, "add-llm-profile");
    ...
    // is unblocked before starting the conversation. Conversations launch from the active AGENT
  ...
  await test.step("run a conversation with the mock LLM", async () => {
  ...
  await test.step("resume conversation from sidebar after navigating away", async () => {
    await routeSessionApiKey(page);
    ...
    const conversationLink = page.locator(
      `a[href*="/conversations/${createdConversationId}"]`,
    );
    ...
    expect(page.url()).toContain(createdConversationId);
