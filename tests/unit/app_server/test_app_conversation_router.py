...
class TestFinalizeSandboxDelete:
    ...
    @pytest.mark.asyncio
    async def test_forwards_pre_delete_run_metrics(self):
        sandbox_service, info_service, db_session, httpx_client = self._deps(0)
        conv = uuid4()
...
@pytest.mark.asyncio
async def test_delete_snapshots_run_metrics_before_agent_conversation_delete():
    ...
    info_service = MagicMock()
    info_service.get_app_conversation_info = AsyncMock(return_value=info)
    info_service.count_conversations_by_sandbox_id = AsyncMock(return_value=1)
    service = MagicMock()
    order = []
    ...
    service.delete_app_conversation = delete
    sandbox_service = MagicMock()
    sandbox_spec_service = MagicMock()
    db_session = AsyncMock()
    httpx_client = AsyncMock()
    ...
    with (
        patch(
            'openhands.app_server.app_conversation.app_conversation_router.'
            'workspace_archive.archive_enabled',
            return_value=True,
        ),
        patch(
            'openhands.app_server.app_conversation.app_conversation_router.'
        ...
        patch(
            'openhands.app_server.app_conversation.app_conversation_router.'
            'workspace_archive._fetch_run_metrics',
            side_effect=fetch,
        ),
        patch(
            'openhands.app_server.app_conversation.app_conversation_router.'
    ...
    assert order == ['fetch', 'delete']
    assert finalize.call_args.kwargs['run_metrics'] == run_metrics
