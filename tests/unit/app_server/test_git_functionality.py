    ...
    async def clone_or_init_git_repo(
        ...
        try:
            ...
            if clone_result.returncode == 0:
                self.logger.info(f"Successfully cloned repository {repo_url} to {workspace_path}")
                return True
            ...
            if checkout_result.returncode == 0:
                ...
                self.logger.error(f"Failed to checkout branch {branch}: {checkout_result.stderr}")
                return False
...
async def test_clone_or_init_git_repo_successful_clone(service):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.assert_called_once_with(
            ["git", "clone", "--branch", "main", "https://github.com/test/repo.git", "/tmp/test_repo"],
            capture_output=True,
            text=True,
            timeout=300,
        )
...
async def test_clone_or_init_git_repo_clone_fails_checkout_succeeds(service):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.side_effect = [
            ...
            MagicMock(returncode=0, stderr="", stdout="Switched to branch"),  # Checkout succeeds
        ]
        ...
        mock_run.assert_any_call(
            ["git", "clone", "--branch", "feature-branch", "https://github.com/test/repo.git", "/tmp/test_repo"],
            capture_output=True,
            text=True,
            timeout=300,
        )
        ...
        service.logger.info.assert_called_with("Successfully checked out branch feature-branch")
...
async def test_clone_or_init_git_repo_both_operations_fail(service):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.side_effect = [
            MagicMock(returncode=1, stderr="Clone failed", stdout=""),  # Clone fails
            MagicMock(returncode=1, stderr="Checkout failed", stdout=""),  # Checkout fails
        ]
...
async def test_clone_or_init_git_repo_timeout(service):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        assert result is False
        service.logger.error.assert_called_with("Git operation timed out after 300 seconds")
...
async def test_clone_or_init_git_repo_exception(service):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        assert result is False
        service.logger.error.assert_called_with("Git operation failed: Unexpected error")
