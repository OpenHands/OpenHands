...
async def test_store_initializes_persistent_opaque_version(store):
    ...
    value, version = await store.load_managed(_MANAGED_NAME)
    second = _store(store.file_store)

    assert value == _ORIGINAL
    assert version != _ORIGINAL
    assert await second.load_managed(_MANAGED_NAME) == (_ORIGINAL, version)


@pytest.mark.asyncio
async def test_load_managed_never_writes_missing_version():
    ...
    with pytest.raises(NotImplementedError):
        await store.load_managed(_MANAGED_NAME)

    assert json.loads(file_store.read('secrets.json')) == raw


@pytest.mark.asyncio
async def test_load_managed_does_not_open_lock_file(store, monkeypatch):
    await store.store(_secrets(_ORIGINAL))
    ...
    assert (await store.load_managed(_MANAGED_NAME))[0] == _ORIGINAL
...
async def test_bootstrap_merges_version_into_raw_document():
    ...
    await store.ensure_managed(_MANAGED_NAME)
    updated = json.loads(file_store.read('secrets.json'))
    version = updated.pop('_credential_versions')[_MANAGED_NAME]

    assert updated == raw
    assert await store.load_managed(_MANAGED_NAME) == (_ORIGINAL, version)
...
async def test_bootstrap_is_stable_across_concurrent_stores(tmp_path):
    ...
    await asyncio.gather(*(store.ensure_managed(_MANAGED_NAME) for store in stores))
    versions = {(await store.load_managed(_MANAGED_NAME))[1] for store in stores}
...
async def test_replace_is_compare_and_swap(store):
    await store.store(_secrets(_ORIGINAL))
    _, version = await store.load_managed(_MANAGED_NAME)

    with pytest.raises(CredentialVersionConflict):
        await store.replace_managed(_MANAGED_NAME, 'stale', _ROTATED)
    successor = await store.replace_managed(_MANAGED_NAME, version, _ROTATED)

    assert successor != version
    assert await store.load_managed(_MANAGED_NAME) == (_ROTATED, successor)
...
async def test_concurrent_replacements_have_one_winner(tmp_path):
    ...
    results = await asyncio.gather(
        *(
            _store(file_store).replace_managed(_MANAGED_NAME, version, value)
...
async def test_replace_preserves_unrecognized_file_data():
    ...
    await store.replace_managed(_MANAGED_NAME, version, _ROTATED)
    updated = json.loads(file_store.read('secrets.json'))
...
async def test_delete_and_identical_recreate_changes_version(store):
    ...
    assert (await store.load_managed(_MANAGED_NAME))[1] != version
