...
async def test_agent_controller_processes_null_observation_with_cause(
    ...
    """Test that AgentController processes workspace recall follow-up observations.

    Memory may emit ``NullObservation`` (empty context) or ``RecallObservation``
    (when repo/microagent content exists). Both should reference the ``RecallAction``
    id as ``cause`` and drive ``should_step`` appropriately.
    """
    mock_agent, conversation_stats, llm_registry = mock_agent_with_stats
    ...
    with patch.object(controller, '_step') as mock_step:
        ...
        # Allow async follow-up after RecallAction (workspace recall may emit NullObservation
        # when empty, or RecallObservation when repo/microagent context exists — e.g. after UTF-8 loads).
        recall_action = None
        workspace_followup = None
        events: list = []
        for _ in range(100):
            await asyncio.sleep(0.05)
            events = list(event_stream.get_events())
            recalls = [e for e in events if isinstance(e, RecallAction)]
            if not recalls:
                continue
            recall_action = recalls[0]
            tagged = [
                e
                for e in events
                if isinstance(e, (NullObservation, RecallObservation))
                and e.cause == recall_action.id
            ]
            if tagged:
                workspace_followup = tagged[0]
                break

        # Event 0: MessageAction … Event 1: RecallAction … Event 2+: ENVIRONMENT observation for recall

        assert recall_action is not None, 'No RecallAction was created'
        assert workspace_followup is not None, (
            'No NullObservation or RecallObservation follow-up for RecallAction'
        )

        assert workspace_followup.cause is not None, (
            'Follow-up observation cause is None'
        )
        assert workspace_followup.cause == recall_action.id, (
            f'Expected cause={recall_action.id}, got cause={workspace_followup.cause}'
        )

        assert controller.should_step(workspace_followup), (
            'should_step should return True for workspace recall follow-up observation'
        )
