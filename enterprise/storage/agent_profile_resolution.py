...
class OrgLLMProfileLoader:
    ...
    The ``org.llm_profiles`` ``SecretAwareJSON`` column decrypts secret leaves at
    the column boundary, so the wrapped ``LLMProfiles`` already holds cleartext
    keys; the ``cipher`` arg is accepted for Protocol parity and ignored.
    """
