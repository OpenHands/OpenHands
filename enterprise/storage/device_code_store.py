...
class DeviceCodeStore:
    ...
    def create_device_code(
        ...
        with self.session_maker() as session:
            ...
            for _ in range(max_attempts):
                ...
                existing_user = session.query(DeviceCode).filter_by(user_code=user_code).first()
                existing_device = session.query(DeviceCode).filter_by(device_code=device_code).first()
                
                if not existing_user and not existing_device:
                    break
            else:
                raise RuntimeError("Failed to generate unique device codes after multiple attempts")
    ...
    def authorize_device_code(
        ...
        """
        with self.session_maker() as session:
            device_code_entry = session.query(DeviceCode).filter_by(user_code=user_code).first()
    ...
    def cleanup_expired_codes(self) -> int:
        ...
        """
        with self.session_maker() as session:
            expired_codes = session.query(DeviceCode).filter(
