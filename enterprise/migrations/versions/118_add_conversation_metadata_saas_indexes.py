...
INC-95: conversation_metadata_saas only had the conversation_id primary-key
index, so every query filtering by user_id or org_id did a full table scan —
the 2nd-heaviest sequential-scan table in prod (~680K seq scans, ~42B rows read
since mid-December).
