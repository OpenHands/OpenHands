...
class TestSaasSQLAppConversationInfoService:
    ...
    @pytest.mark.asyncio
    async def test_foreign_preassigned_id_does_not_overwrite_metadata(
        self,
        async_session_with_users: AsyncSession,
    ):
        user1_service = SaasSQLAppConversationInfoService(
        ...
        original = AppConversationInfo(
            id=conversation_id,
            created_by_user_id=str(USER2_ID),
            sandbox_id='user-2-sandbox',
            title='User 2 conversation',
        )
        await user2_service.save_app_conversation_info(original)
        updated = original.model_copy(update={'title': 'User 2 updated conversation'})
        await user2_service.save_app_conversation_info(updated)

        collision = AppConversationInfo(
            id=conversation_id,
            created_by_user_id=str(USER1_ID),
            sandbox_id='user-1-sandbox',
            title='User 1 collision',
        )
        with pytest.raises(AuthError):
            await user1_service.save_app_conversation_info(collision)
        ...
        assert stored.sandbox_id == original.sandbox_id

        loaded = await user2_service.get_app_conversation_info(conversation_id)
        assert loaded is not None
        assert loaded.created_by_user_id == str(USER2_ID)

    @pytest.mark.asyncio
    async def test_delete_removes_owner_and_allows_id_reuse(
        self,
        async_session_with_users: AsyncSession,
    ):
        service = SaasSQLAppConversationInfoService(
        ...
        assert await service.delete_app_conversation_info(conversation_id)
        assert (
            await async_session_with_users.get(
                StoredConversationMetadata, str(conversation_id)
            )
            is None
        )
        assert (
            await async_session_with_users.get(
                StoredConversationMetadataSaas, str(conversation_id)
            )
            is None
        )
        assert await service.try_reserve_app_conversation_id(
            conversation_id, str(USER1_ID)
        )

    @pytest.mark.asyncio
    async def test_preassigned_id_reservation_is_atomic_across_orgs(
        self,
        async_session_with_users: AsyncSession,
    ):
        org1_context = MagicMock()
        org1_context.get_user_id = AsyncMock(return_value=str(USER1_ID))
        org1_context.get_effective_org_id = AsyncMock(return_value=ORG1_ID)
        org2_context = MagicMock()
        org2_context.get_user_id = AsyncMock(return_value=str(USER1_ID))
        org2_context.get_effective_org_id = AsyncMock(return_value=ORG2_ID)
        org1_service = SaasSQLAppConversationInfoService(
            db_session=async_session_with_users,
            user_context=org1_context,
        )
        org2_service = SaasSQLAppConversationInfoService(
            db_session=async_session_with_users,
            user_context=org2_context,
        )
        conversation_id = uuid4()

        assert await org1_service.try_reserve_app_conversation_id(
            conversation_id, str(USER1_ID)
        )
        async_session_with_users.expunge_all()
        assert not await org2_service.try_reserve_app_conversation_id(
            conversation_id, str(USER1_ID)
        )
        ...
        owner = await async_session_with_users.get(
            StoredConversationMetadataSaas, str(conversation_id)
        )
        assert stored is not None
        assert stored.conversation_version == APP_CONVERSATION_RESERVATION_VERSION
        assert owner is not None
        assert owner.user_id == USER1_ID
        assert owner.org_id == ORG1_ID

        conversation = AppConversationInfo(
            id=conversation_id,
            created_by_user_id=str(USER1_ID),
            sandbox_id='shared-sandbox',
            title='Reserved conversation',
        )
        await org1_service.save_app_conversation_info(conversation)
        await org1_service.release_app_conversation_id_reservation(conversation_id)

        loaded = await org1_service.get_app_conversation_info(conversation_id)
        assert loaded is not None
    ...
    @pytest.mark.asyncio
    async def test_stale_reservation_reclaim_rejects_old_claim(
        self,
        async_session_with_users: AsyncSession,
    ):
        context = MagicMock()
        context.get_user_id = AsyncMock(return_value=str(USER1_ID))
        context.get_effective_org_id = AsyncMock(return_value=ORG1_ID)
        old_service = SaasSQLAppConversationInfoService(
            db_session=async_session_with_users,
            user_context=context,
        )
        replacement_service = SaasSQLAppConversationInfoService(
            db_session=async_session_with_users,
            user_context=context,
        )
        conversation_id = uuid4()
        assert await old_service.try_reserve_app_conversation_id(
            conversation_id, str(USER1_ID)
        )
        stored = await async_session_with_users.get(
            StoredConversationMetadata, str(conversation_id)
        ...
        assert await replacement_service.try_reserve_app_conversation_id(
            conversation_id, str(USER1_ID)
        )
        stored = await async_session_with_users.get(
            StoredConversationMetadata, str(conversation_id)
        ...
        await old_service.release_app_conversation_id_reservation(conversation_id)
        ...
        with pytest.raises(ValueError, match='reservation is not owned'):
            await old_service.save_app_conversation_info(conversation)
        await async_session_with_users.rollback()

        await replacement_service.save_app_conversation_info(conversation)
        await replacement_service.release_app_conversation_id_reservation(
            conversation_id
        )
        loaded = await replacement_service.get_app_conversation_info(conversation_id)
        assert loaded is not None

    @pytest.mark.asyncio
    async def test_managed_sandbox_lookup_spans_owner_orgs(
        self,
        async_session_with_users: AsyncSession,
    ):
        org1_context = MagicMock()
        ...
        tag = {CODEX_CREDENTIAL_BINDING_TAG_KEY: CODEX_CREDENTIAL_BINDING_TAG_VALUE}
        org1_conversation = AppConversationInfo(
            id=uuid4(),
            created_by_user_id=str(USER1_ID),
            sandbox_id='shared-sandbox',
            tags=tag,
        )
        org2_conversation = AppConversationInfo(
            id=uuid4(),
            created_by_user_id=str(USER1_ID),
            sandbox_id='shared-sandbox',
            tags=tag,
        )
        await org1_service.save_app_conversation_info(org1_conversation)
        await org2_service.save_app_conversation_info(org2_conversation)

        refs = await org1_service.get_managed_credential_conversations_for_sandbox(
            'shared-sandbox'
        )

        assert {
            (ref.conversation_id, ref.created_by_user_id, ref.organization_id)
            for ref in refs
        } == {
...
class TestSaasSQLAppConversationInfoServiceWebhookFallback:
    ...
    @pytest.mark.asyncio
    async def test_admin_context_updates_existing_owner(
        self,
        async_session_with_users: AsyncSession,
    ):
        from openhands.app_server.user.specifiy_user_context import ADMIN
        ...
        conversation = AppConversationInfo(
            id=uuid4(),
            created_by_user_id=str(USER1_ID),
            sandbox_id='sandbox-webhook-update',
        ...
        await admin_service.save_app_conversation_info(
            conversation.model_copy(update={'title': 'After webhook'})
        )
        ...
        assert stored is not None
        assert stored.title == 'After webhook'

    @pytest.mark.asyncio
    async def test_sandbox_context_rejects_missing_organization(
        self,
        async_session_with_users: AsyncSession,
    ):
        conversation_id = uuid4()
        async_session_with_users.add(
            StoredConversationMetadata(
                conversation_id=str(conversation_id),
                conversation_version='V1',
                sandbox_id='sandbox-webhook-backfill',
            )
        )
        await async_session_with_users.commit()
        user = await async_session_with_users.get(User, USER1_ID)
        ...
        user.current_org_id = ORG2_ID
        await async_session_with_users.commit()
        sandbox_service = SaasSQLAppConversationInfoService(
            db_session=async_session_with_users,
            user_context=SandboxUserContext(
                user_id=str(USER1_ID),
                sandbox_id='sandbox-webhook-backfill',
        ...
        with pytest.raises(AuthError, match='organization is unavailable'):
            await sandbox_service.save_app_conversation_info(
                AppConversationInfo(
                    id=conversation_id,
                    created_by_user_id=str(USER1_ID),
                    sandbox_id='sandbox-webhook-backfill',
    ...
    @pytest.mark.asyncio
    async def test_sandbox_context_creates_new_conversation(
        self,
        async_session_with_users: AsyncSession,
    ):
        conversation_id = uuid4()
        sandbox_service = SaasSQLAppConversationInfoService(
            ...
            user_context=SandboxUserContext(
                user_id=str(USER1_ID),
                sandbox_id='sandbox-webhook-create',
            ),
        )
...
class TestResolverOrgIdRouting:
    ...
    async def test_save_uses_resolver_org_id_when_set_on_context(
        ...
        mock_context.get_effective_org_id = AsyncMock(return_value=ORG2_ID)
        ...
        assert saas_metadata.user_id == USER1_ID
        conversation = await service.get_app_conversation_info(conv_id)
    ...
    async def test_save_uses_default_org_when_resolver_org_id_is_none(
        ...
        mock_context = MagicMock(spec=ResolverUserContext)
        mock_context.get_user_id = AsyncMock(return_value=str(USER1_ID))
        mock_context.get_effective_org_id = AsyncMock(return_value=None)
