...
class TestGetOrCreateApiKeyForUser:
    ...
    @pytest.mark.asyncio
    async def test_user_id_mismatch(self, valid_request):
        """Test error when path user_id doesn't match body user_id."""
        ...
        assert 'user_id in path must match' in exc_info.value.detail
    ...
    @pytest.mark.asyncio
    async def test_user_not_in_org(self, valid_request):
    ...
    @pytest.mark.asyncio
    async def test_successful_key_creation(self, valid_request):
        ...
        assert response.org_id == str(valid_request.org_id)
        ...
        mock_api_key_store.get_or_create_system_api_key.assert_called_once_with(
            user_id=valid_request.user_id,
            org_id=valid_request.org_id,
            name='automation',
        )

    @pytest.mark.asyncio
    async def test_store_exception_handling(self, valid_request):
