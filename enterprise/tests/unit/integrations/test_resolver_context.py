...
from openhands.sdk.conversation.secret_source import StaticSecret, SecretSource
...
@pytest.mark.asyncio
async def test_get_secrets_converts_custom_to_static(resolver_context, mock_saas_user_auth):
...
@pytest.mark.asyncio
async def test_get_secrets_with_special_characters(resolver_context, mock_saas_user_auth):
...
@pytest.mark.asyncio
@pytest.mark.parametrize("secrets_input,expected_result", [
    (None, {}),  # No secrets available
    (create_secrets({}), {}),  # Empty custom secrets
])
async def test_get_secrets_empty_cases(resolver_context, mock_saas_user_auth, secrets_input, expected_result):
