...
# Tests for target_org_id parameter


@pytest.mark.asyncio
async def test_get_org_app_settings_with_target_org_id(
    user_id, mock_org, mock_store, mock_user_context
):
    """
    GIVEN: A specific org_id is provided
    WHEN: get_org_app_settings is called with target_org_id
    THEN: OrgAppSettingsResponse is returned for that specific org
    """
    # Arrange
    target_org_id = mock_org.id
    service = OrgAppSettingsService(store=mock_store, user_context=mock_user_context)

    mock_store.get_org_by_id = AsyncMock(return_value=mock_org)

    # Act
    result = await service.get_org_app_settings(target_org_id=target_org_id)
...
@pytest.mark.asyncio
async def test_get_org_app_settings_target_org_not_found(
    user_id, mock_store, mock_user_context
):
    """
    GIVEN: A non-existent org_id is provided
    WHEN: get_org_app_settings is called with target_org_id
    THEN: OrgNotFoundError is raised for the target org
    """
    # Arrange
    target_org_id = uuid.uuid4()
    service = OrgAppSettingsService(store=mock_store, user_context=mock_user_context)

    mock_store.get_org_by_id = AsyncMock(return_value=None)

    # Act & Assert
    with pytest.raises(OrgNotFoundError) as exc_info:
        await service.get_org_app_settings(target_org_id=target_org_id)

    assert str(target_org_id) in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_org_app_settings_with_target_org_id(
    user_id, mock_org, mock_store, mock_user_context
):
    """
    GIVEN: A specific org_id is provided
    WHEN: update_org_app_settings is called with target_org_id
    THEN: Org is updated and returned for that specific org
    """
    # Arrange
    target_org_id = mock_org.id
    update_data = OrgAppSettingsUpdate(enable_proactive_conversation_starters=False)
    ...
    updated_org.id = target_org_id
    updated_org.enable_proactive_conversation_starters = False
    updated_org.max_budget_per_task = 25.0

    service = OrgAppSettingsService(store=mock_store, user_context=mock_user_context)

    mock_store.get_org_by_id = AsyncMock(return_value=mock_org)
    mock_store.update_org_app_settings = AsyncMock(return_value=updated_org)

    # Act
    result = await service.update_org_app_settings(
        update_data, target_org_id=target_org_id
    )
...
@pytest.mark.asyncio
async def test_update_org_app_settings_target_org_not_found(
    user_id, mock_store, mock_user_context
):
    """
    GIVEN: A non-existent org_id is provided
    ...
    # Act & Assert
    with pytest.raises(OrgNotFoundError) as exc_info:
        await service.update_org_app_settings(update_data, target_org_id=target_org_id)

    assert str(target_org_id) in str(exc_info.value)


@pytest.mark.asyncio
async def test_target_org_id_takes_precedence_over_effective_org(
    user_id, mock_org, mock_store, mock_user_context
):
    """
    GIVEN: Both target_org_id and effective org mechanism are available
    WHEN: get_org_app_settings is called with target_org_id
    THEN: target_org_id takes precedence and effective org is not resolved
    """
    # Arrange
    target_org_id = uuid.uuid4()
    different_org = MagicMock(spec=Org)
    different_org.id = target_org_id
    different_org.enable_proactive_conversation_starters = False
    different_org.max_budget_per_task = 100.0

    service = OrgAppSettingsService(store=mock_store, user_context=mock_user_context)

    # Setup both paths
    mock_store.get_org_by_id = AsyncMock(return_value=different_org)
    mock_store.get_current_org_by_user_id = AsyncMock(return_value=mock_org)
