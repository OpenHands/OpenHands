...
from integrations.linear.routes import (
    LinearLinkCreate,
...
@pytest.mark.asyncio
@patch('integrations.linear.routes.linear_manager', new_callable=AsyncMock)
@patch('integrations.linear.routes.redis_client', new_callable=MagicMock)
async def test_linear_events_processing_success(mock_redis, mock_manager, mock_request):
    with patch('integrations.linear.routes.LINEAR_WEBHOOKS_ENABLED', True):
...
@pytest.mark.asyncio
@patch('integrations.linear.routes.get_user_auth')
...
@pytest.mark.asyncio
@patch('integrations.linear.routes.get_user_auth')
@patch('integrations.linear.routes.redis_client')
...
@patch('integrations.linear.routes.linear_manager', new_callable=AsyncMock)
@patch(
    'integrations.linear.routes._handle_workspace_link_creation',
...
async def test_linear_callback_workspace_integration_existing_workspace(
    ...
    with patch('integrations.linear.routes.token_manager') as mock_token_manager:
        with patch(
            'integrations.linear.routes._validate_workspace_update_permissions'
        ) as mock_validate:
