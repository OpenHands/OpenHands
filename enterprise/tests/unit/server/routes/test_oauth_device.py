...
class TestDeviceToken:
    ...
    @patch('server.routes.oauth_device.device_code_store')
    async def test_device_token_error_cases(self, mock_store, device_exists, status, expected_error):
...
class TestKeycloakCallback:
    ...
    @patch('server.routes.oauth_device.config')
    async def test_keycloak_callback_success(self, mock_config, mock_token_mgr, mock_store, mock_api_key_class):
        ...
        # Mock token manager
        mock_token_mgr.get_keycloak_tokens = AsyncMock(return_value=('access-token', 'refresh-token'))
        ...
        with patch('server.routes.oauth_device.jwt.decode', return_value={'user_code': 'ABC12345'}):
