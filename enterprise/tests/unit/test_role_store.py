...
@pytest.mark.asyncio
async def test_get_role_by_id(async_session_maker):
...
@pytest.mark.asyncio
async def test_get_role_by_name(async_session_maker):
    ...
    # Test retrieval
    with patch('storage.role_store.a_session_maker', async_session_maker):
        retrieved_role = await RoleStore.get_role_by_name('admin')
        assert retrieved_role is not None
        assert retrieved_role.id == role_id
        assert retrieved_role.name == 'admin'
...
@pytest.mark.asyncio
async def test_get_role_by_name_not_found(async_session_maker):
...
@pytest.mark.asyncio
async def test_list_roles(async_session_maker):
...
@pytest.mark.asyncio
async def test_create_role(async_session_maker):
...
async def test_create_role_with_session(async_session_maker):
    ...
    async with async_session_maker() as session:
        role = await RoleStore.create_role(name='editor', rank=3, session=session)
