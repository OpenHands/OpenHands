...
class SlackUpdateExistingConversationView(SlackNewConversationView):
    ...
    async def send_message_to_v1_conversation(self, jinja: Environment):
        ...
        slack_user_context = ResolverUserContext(
            saas_user_auth=self.saas_user_auth,
            resolver_org_id=self.slack_conversation.org_id,
        )
        setattr(state, USER_CONTEXT_ATTR, slack_user_context)
        ...
        async with (
            ...
            async def resume_conversation() -> None:
                ...
                if terminal_task is None:
                    raise RuntimeError('Conversation resume ended without a result')
                if terminal_task.status == AppConversationStartTaskStatus.ERROR:
                    raise RuntimeError(
