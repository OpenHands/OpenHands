...
class JiraDcV1CallbackProcessor(EventCallbackProcessor):
    ...
    jira_dc_view_data: dict[str, Any] = Field(default_factory=dict)
    ...
    async def __call__(
        ...
        _logger.info('[Jira DC V1] Callback agent state was %s', event)
        _logger.info(
            '[Jira DC V1] Should request summary: %s', self.should_request_summary
        )
        ...
        try:
            _logger.info(f'[Jira DC V1] Requesting summary {conversation_id}')
            summary_markdown = await self._request_summary(conversation_id)

            # Convert markdown to Jira wiki markup
            summary = markdown_to_jira_markup(summary_markdown)

            _logger.info(
                f'[Jira DC V1] Posting summary {conversation_id}',
            ...
            _logger.exception('[Jira DC V1] Error processing callback: %s', e)

            # Try to post error to Jira DC
            try:
                await self._post_summary_to_jira_dc(
                ...
                _logger.warning(
                    '[Jira DC V1] Failed to post error message to Jira DC: %s',
                    post_error,
                )
    ...
    async def _post_summary_to_jira_dc(self, summary: str) -> None:
        ...
        if not all([issue_key, workspace_name, base_api_url]):
            raise RuntimeError(
                'Missing required Jira DC view data '
                f'(issue_key={issue_key}, workspace_name={workspace_name}, '
                f'base_api_url={base_api_url})'
            )
        ...
        _logger.info('[Jira DC V1] Successfully posted summary to issue %s', issue_key)
    ...
    async def _ask_question(
        ...
        try:
            ...
            _logger.error(
                '[Jira DC V1] HTTP error sending message to %s: %s. '
            ...
            _logger.error(
                '[Jira DC V1] %s. Request payload: %s',
                error_detail,
                payload,
                exc_info=True,
            )
