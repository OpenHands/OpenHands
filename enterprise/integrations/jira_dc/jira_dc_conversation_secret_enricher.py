...
def _email_mode_hint() -> str:
    ...
    return (
        f'Jira Data Center for this workspace is at {JIRA_DC_BASE_URL}, but the '
        'admin has enabled email-linking mode so no Jira token is automatically '
        'provided to conversations. If the user asks you to read or query Jira '
        'Data Center, check to see if you have an available Jira token (e.g. a '
        'Secret) or Jira MCP tools in your environment; otherwise, tell the user '
        'how to enable access: an admin can enable OAuth mode in the OHE Admin '
        'Config and redeploy, or the user can add their Jira API token as a '
        'Secret, or connect a Jira Data Center MCP server (Settings → MCP).'
    )


def _append_email_mode_hint(system_message_suffix: str | None) -> str:
    hint = _email_mode_hint()
    ...
    async def enrich(
        ...
        *,
        ...
        if not JIRA_DC_ENABLE_OAUTH:
            # Email mode: no per-user token to inject. When the connection is set
            # up, tell the agent how to point the user at enabling live access
            # instead of failing silently.
            store = JiraDcIntegrationStore.get_instance()
            if await store.get_active_workspace():
                return ConversationSecretEnrichment(
                    system_message_suffix=_append_email_mode_hint(system_message_suffix)
                )
            return ConversationSecretEnrichment(
