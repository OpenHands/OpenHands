...
from integrations.linear.plugin import LinearPluginConfig
...
# ---------------------------------------------------------------------------
# Module-level state (initialized by create_linear_router)
# ---------------------------------------------------------------------------
...
async def _handle_workspace_link_creation(
    user_id: str, linear_user_id: str | None, target_workspace: str
):
...
async def create_linear_workspace(
    ...
    try:
        ...
        auth_params = {
            'client_id': _plugin_config.client_id,
            'redirect_uri': _linear_redirect_uri,
            'scope': LINEAR_SCOPES,
            'state': state,
            'response_type': 'code',
        }
...
async def linear_callback(request: Request, code: str, state: str):
    ...
    token_payload = {
        ...
        'client_id': _plugin_config.client_id,
        'client_secret': _plugin_config.client_secret,
        'code': code,
        'redirect_uri': _linear_redirect_uri,
    }
    ...
    if response.status_code != 200:
        raise HTTPException(
            status_code=400,
            detail=f'Error fetching workspace: {response.text}',
        )
...
def create_linear_router(plugin_config) -> APIRouter:
    """Create a self-contained FastAPI router for Linear integration.

    Initialises module-level state (``linear_manager``, ``redis_client``,
    ``token_manager``, etc.) and registers all endpoints on a new router.

    Args:
        plugin_config: A ``LinearPluginConfig`` instance with all settings.

    Returns:
        Configured ``APIRouter`` with all Linear endpoints.
    """
    global \
        LINEAR_WEBHOOKS_ENABLED, \
        _plugin_config, \
        linear_manager, \
        redis_client, \
        token_manager, \
    ...
    cfg: LinearPluginConfig = plugin_config

    _plugin_config = cfg
    LINEAR_WEBHOOKS_ENABLED = cfg.webhooks_enabled
    token_manager = TokenManager()
    linear_manager = LinearManager(token_manager)
    redis_client = create_redis_client()
    _linear_redirect_uri = f'https://{cfg.web_host}/integration/linear/callback'

    router = APIRouter(prefix='/integration/linear')
    router.add_api_route('/events', linear_events, methods=['POST'])
    router.add_api_route('/workspaces', create_linear_workspace, methods=['POST'])
    router.add_api_route('/workspaces/link', create_workspace_link, methods=['POST'])
    router.add_api_route('/callback', linear_callback, methods=['GET'])
    ...
    router.add_api_route('/workspaces/unlink', unlink_workspace, methods=['POST'])
    router.add_api_route(
