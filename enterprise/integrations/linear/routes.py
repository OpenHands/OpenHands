...
def create_linear_router(plugin_config) -> APIRouter:
    ...
    linear_redirect_uri = f'https://{cfg.web_host}/integration/linear/callback'
    ...
    async def _handle_workspace_link_creation(
        ...
        existing_user = (
            await linear_manager.integration_store.get_user_by_active_workspace(
                user_id
            )
        )
        ...
        existing_link = await linear_manager.integration_store.get_user_by_keycloak_id_and_workspace(
    ...
    async def _validate_workspace_update_permissions(
        ...
        if (
            current_user_link
            and current_user_link.linear_workspace_id != workspace.id
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    async def linear_events(
        ...
        try:
            signature_valid, signature, payload = (
                await linear_manager.validate_request(request)
            )
            ...
            if keyExists:
                logger.info(
                    f'Received duplicate Linear webhook event: {signature}'
                )
    ...
    async def create_workspace_link(
        request: Request, link_data: LinearLinkCreate
    ):
    ...
    async def linear_callback(request: Request, code: str, state: str):
        ...
        response = requests.post(
            LINEAR_GRAPHQL_URL, json=graphql_query, headers=headers
        )
        ...
        linear_user_id = (
            workspace_data.get('data', {}).get('viewer', {}).get('id')
        )

        if (
            integration_session.get('operation_type')
            == 'workspace_integration'
        ):
            workspace = (
                await linear_manager.integration_store.get_workspace_by_name(
                    target_workspace
                )
            )
            if not workspace:
    ...
    async def get_current_workspace_link(request: Request):
        ...
        try:
            ...
            workspace = (
                await linear_manager.integration_store.get_workspace_by_id(
