...
class OrgMemberService:
    ...
    async def remove_org_member(
        ...
        # Perform the removal
        success = await OrgMemberStore.remove_user_from_org_async(
            org_id, target_user_id
        )
        if not success:
    ...
    async def update_org_member(
        ...
        # Perform the update
        updated_member = await OrgMemberStore.update_user_role_in_org_async(
    ...
    async def _is_last_owner(org_id: UUID, user_id: UUID) -> bool:
        ...
        members = await OrgMemberStore.get_org_members_async(org_id)
        owners = []
        for m in members:
